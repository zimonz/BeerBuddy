  ...
  <v-container v-if="rankings.length > 0" fluid>
    <v-list three-line subheader>
      ...
      <template v-for="ranking in rankings">
        <v-list-item
          :key="ranking.originalTimeframe.id"
          :style="((selectedTimeFrame && ranking.originalTimeframe.id == selectedTimeFrame.id) ? 'background: rgb(240,240,240)' : '')"
        >
          <v-list-item-content>
            <v-list-item-title
              v-text="formatDateTime(ranking.from) + ' - ' + formatDateTime(ranking.to)"
            ></v-list-item-title>
            <v-list-item-subtitle v-text="'Number of participants ' + ranking.numberOfParticipants"></v-list-item-subtitle>
            <v-list-item-subtitle
              v-text="'Average time per participant ' + ranking.avgTimePerParticipant"
            ></v-list-item-subtitle>
          </v-list-item-content>
          <v-list-item-action>
            <v-btn
              v-if="selectedTimeFrame == null"
              @click="setSelectedTimeframe(ranking.originalTimeframe.id)"
              icon
            >
              select
            ...
            <v-btn
              v-if="selectedTimeFrame != null  && ranking.originalTimeframe.id == selectedTimeFrame.id"
              icon
            >
            ...
            <v-btn
              v-if="selectedTimeFrame != null  && ranking.originalTimeframe.id == selectedTimeFrame.id"
              @click="downloadCal(ranking.from, ranking.to)"
              icon
            >
